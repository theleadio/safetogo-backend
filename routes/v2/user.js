...
router.post('/signin',async function(req, res, next){
    ...
    try{
      ...
      if(result.length > 0){
        let votes = await getUserVotes(result[0]["user_id"]);
        result = {
  ...
  async function getUserVotes(user_id){
    const conn = db.conn.promise();
    let query = ''
    const args = []
    query = `SELECT district, country, reference, case_id, upvote, downvote FROM ${DB_NAME}.votes WHERE user_id='${user_id}'`
